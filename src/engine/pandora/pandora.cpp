...
void Pandora::Upload(std::vector<int> a, int b, std::vector<int> c) {
    this->b = b;
    vmaCopyMemoryToAllocation(allocator, a.data(), storageAllocations[0], 0, bufferSize);
    vmaCopyMemoryToAllocation(allocator, c.data(), storageAllocations[1], 0, bufferSize);
}

std::vector<int> Pandora::Download() {
    std::vector<int> result(bufferSize / sizeof(int));
    vmaCopyAllocationToMemory(allocator, storageAllocations[2], 0, result.data(), bufferSize);
    return result;
}
...
void Pandora::Dispatch(uint32_t x, uint32_t y, uint32_t z) {
    ...
    // RECORDING START
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, calculationPipeline);
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                            calculationPipelineLayout, 0, 1, &descriptorSet, 0, nullptr);
    vkCmdPushConstants(commandBuffer, calculationPipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0,
                       sizeof(int), &b);
    ...
    auto submitInfo = VKToolkit::SubmitInfo2(&commandSubmitInfo, nullptr, nullptr);
...
void Pandora::initInstance(bool headless) {
    vkb::InstanceBuilder builder;

    auto builtInstance =
        builder.use_default_debug_messenger().request_validation_layers().require_api_version(1, 3,
                                                                                              0);
    if (headless) {
        builtInstance = builtInstance.set_headless();
    }
    vkb::Result<vkb::Instance> instanceRet = builtInstance.build();
...
void Pandora::initDescriptor() {
    ...
    VkDescriptorPoolSize poolSize = {
        ...
        3 // describe the total bindings used
    };
    ...
    VkDescriptorSetLayoutBinding bindings[3] = {
        VKToolkit::DescriptorSetLayoutBinding(0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                              VK_SHADER_STAGE_COMPUTE_BIT),
        VKToolkit::DescriptorSetLayoutBinding(1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                              VK_SHADER_STAGE_COMPUTE_BIT),
        VKToolkit::DescriptorSetLayoutBinding(2, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                              VK_SHADER_STAGE_COMPUTE_BIT),
    };
    ...
    VkDescriptorSetAllocateInfo allocateInfo =
        VKToolkit::DescriptorSetAllocateInfo(1, &descriptorSetLayout, descriptorPool);
...
void Pandora::initPipeline(const std::string& shaderPath) {
    ...
    auto computeLayout = VKToolkit::PipelineLayoutCreateInfo(&descriptorSetLayout);
    computeLayout.pushConstantRangeCount = 1;
    computeLayout.pPushConstantRanges = &pushRange;
    vkCreatePipelineLayout(device, &computeLayout, nullptr, &calculationPipelineLayout);

    // Load compiled shader
    auto buffer = FileLoader::LoadBinaryFile(shaderPath);
    // Create shader module
    VkShaderModuleCreateInfo shaderModuleCreateInfo = VKToolkit::ShaderModuleCreateInfo(buffer);
    ...
    auto computePipelineCreateInfo =
        VKToolkit::ComputePipelineCreateInfo(calculationPipelineLayout, computeShaderStage);
    vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &computePipelineCreateInfo, nullptr,
                             &calculationPipeline);
